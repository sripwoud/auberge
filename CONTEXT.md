...
- Every task that replaces an artifact on the version-bump path notifies the restart of each unit running out of what it replaced; `tests/install_notifies_restart.rs` fails the build when one does not, so a bump cannot quietly land new bytes beside the process still running the old ones (#599). Scoped to that path because an artifact that is _missing_ leaves its unit dead, which the `state: started` further down revives; and exempting the `oneshot` a timer drives, which execs the artifact afresh at every activation.
