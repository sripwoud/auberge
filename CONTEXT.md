...
- A **Path Attestation** and the role that declares the same path cannot drift apart unnoticed: `tests/grimmory_role.rs` fails the build when a directory the grimmory role creates for the service user is missing from the unit's `ReadWritePaths`, or when the Backup Recipe's paths stop matching the role's data directories. Role-scoped, not fleet-wide — of the 13 service-owned directories across the fleet their unit cannot write, all 13 are deliberate least privilege, so the invariant is grimmory's rather than everyone's (#624, ADR-0033).
