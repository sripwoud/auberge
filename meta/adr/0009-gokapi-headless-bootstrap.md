...
**2. Run the password one-shot once per host.** A new task invokes `{{ gokapi_install_path }}/gokapi --deployment-password '{{ gokapi_admin_password }}'` as the `gokapi` system user, gated on the absence of a marker file at `{{ gokapi_data_dir }}/.bootstrap_done`. Gokapi exits 0 immediately after `EditSuperAdmin` finishes. A subsequent task creates the marker so the one-shot runs only once.
...
- Operators who change `gokapi_admin_password` in their config and re-deploy will not see the new password applied until they `rm /var/lib/gokapi/.bootstrap_done` on the host and re-deploy. Rotating the password is a deliberate operator action, not a deploy side effect. Documented in the role's `defaults/main.yml`.
