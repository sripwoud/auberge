...
Two orderings shifted, both in the direction of a tidier terminal, neither observable in practice. A failed prune now clears its bar before reporting, where it used to leave it spinning; and prune's `--verbose` subprocess lines print before the bar clears rather than after, on a stderr stream `restic forget` leaves empty.
