# ADR-0066: A backup's quiesce window is guarded by a host-side deadman, not the driver alone
...
Scoped per-App: each `RecipeExecutor::backup` call arms and disarms its own timer, matching the granularity the executor already restarts at. Restore's identical gap is deferred to #775 — restore is manually invoked and typically attended, unlike the unattended `backup` path that produced two real outages (36h, 11h).

A fire is not silent: it leaves a marker the _next_ `auberge backup` run reads and reports (warn, not hard-fail — a missed window failing the next scheduled run too would cascade one outage into two).
...
- **Restore carries the identical gap, unaddressed here.** Tracked as #775.
...
- Issue #775 — restore's identical quiesce gap, deferred.
