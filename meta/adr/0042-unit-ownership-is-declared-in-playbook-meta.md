...
- "Active since before this deploy" vs "restarted during the deploy window" is computed, never asserted: the unit's monotonic start timestamp against the Host's own `/proc/uptime` and the locally measured deploy duration — one clock, so no skew between operator and Host. The report deliberately claims nothing about which artifact runs — a restart timestamp cannot know whether the artifact swap had happened yet — and the raw `ActiveState (SubState)` stays on every line so a wrong verdict is auditable. A timer-fired oneshot between ticks is `inactive (dead)` by design, so a `.service` whose `.timer` sibling is in the probe set rolls up as idle instead of crying "stopped" on five healthy Apps.
