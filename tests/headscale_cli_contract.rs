...
fn the_verified_cli_version_is_the_one_the_playbook_pins() {
    ...
    assert_eq!(
        ...
        "\n\
         ...
         `users destroy --help`, `auth register --help`, `nodes tag --help` — plus \
         the JSON its `-o json` prints. Four claims live outside `--help` and must \
         be re-read with it, all in `hscontrol`: `registerCacheExpiration` in \
         `state/state.go`, whose 15 minutes register's error message quotes; the \
         wording of `REGISTRATION_CACHE_MISS` and of `TAG_NOT_IN_POLICY` — prose \
         auberge matches on, so a reworded release ends a translation with every \
         test still green (#729, #769); and `SetNodeTags` in `state/state.go`, \
         which decides both whether `nodes tag` replaces or appends a tag set \
         (0.29.3: assigns it wholesale, contradicting its own `--help`) and \
         whether it still gates tags on the policy's `tagOwners`. Update \
